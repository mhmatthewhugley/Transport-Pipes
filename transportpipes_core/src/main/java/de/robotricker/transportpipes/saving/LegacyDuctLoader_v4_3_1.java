        ...
        for (Duct duct : ductCompoundTagMap.keySet()) {
            ...
            if (duct.getDuctType().getBaseDuctType().is("pipe")) {
                ...
                for (CompoundTag pipeItemTag : ductTag.getListTag("PipeItems").asCompoundTagList()) {
                    ...
                    RelativeLocation relLoc = new RelativeLocation(Float.parseFloat(relLocString.split(":")[0]), Float.parseFloat(relLocString.split(":")[1]),
                            Float.parseFloat(relLocString.split(":")[2]));
